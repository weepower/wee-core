...
export class Store {
    ...
    _storage(obj, key, create = false) {
        let data = obj;
        const type = $type(key);
        const num = type == 'number'; // eslint-disable-line eqeqeq
        let val;

        if (num || type == 'string') { // eslint-disable-line eqeqeq
            const segs = key.toString().split('.');
            key = segs.pop();
            data = data.$;
...
function _setVar(instance, el) {
    ...
    key.slice(-2) == '[]' ? // eslint-disable-line eqeqeq
