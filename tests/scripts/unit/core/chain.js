...
describe('Core: Chain', () => {
    before(() => {
        ...
        div.innerHTML = 'test';
        div.className = 'test';
        _doc.querySelector('body').appendChild(div);
    });

    after(() => {
        _doc.querySelector('body').innerHTML = '';
    });
    ...
    describe('reverse', () => {
        before(() => {
            let html = `<div>
                ...
                fragment = _doc.createRange().createContextualFragment(html);

            _doc.querySelector('body').appendChild(fragment);
        });
