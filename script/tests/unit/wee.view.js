define(function(require) {
	...
	registerSuite({
		...
		'render': function() {
			...
			assert.strictEqual(Wee.view.render('<ul>{{#children}}<li>{{firstName}}</li>{{/children}}</ul>', data), '<ul><li>Tim</li><li>Kathy</li></ul>',
		...
		'addFilter': function() {
			...
			assert.strictEqual(Wee.view.render('{{#numbers|range(2, 4)}}{{.}}{{/numbers}}', data), '34',
